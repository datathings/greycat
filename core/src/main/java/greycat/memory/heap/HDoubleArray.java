...
import greycat.memory.Array;
import greycat.memory.DoubleArray;
import greycat.memory.Struct;

final class HDoubleArray implements DoubleArray {

    private double[] backend = null;
    private int used = 0;
    private final Struct _host;

    HDoubleArray(final Struct host) {
        this._host = host;
    }
    ...
    @Override
    public Array<Double> set(int offset, Double elem) {
        if (backend == null || offset >= used) {
            throw new RuntimeException("allocate first!");
        } else {
            backend[offset] = elem;
            _host.chunk().setDirty();
        }
        return this;
    }
    ...
    final HDoubleArray cloneFor(HeapContainer target) {
        HDoubleArray cloned = new HDoubleArray(target);
        if (_backend != null) {
