...
public class Gamma {
    ...
    public static double factorial(int n) {
        if (n < 0) {
            throw new RuntimeException("n has to be nonnegative.");
        }
    ...
    public static double logFactorial(int n) {
        if (n < 0) {
            throw new RuntimeException("n has to be non-negative");
        }
    ...
    public static double lgamma(double x) {
        ...
        if (x >= 0.0) {
            ...
            if (fg != 1.0 / 0.0 && fg != -1.0 / 0.0) {
                if (fg < 0) {
                    throw new RuntimeException("The gamma function is negative");
                } else {
                    fg = Math.log(fg);
                }
            }
    ...
    public static double regularizedIncompleteGamma(double s, double x) {
        if (s < 0.0) {
            throw new RuntimeException("Invalid s: " + s);
        }
    ...
    private static double regularizedIncompleteGammaSeries(double a, double x) {
        if (a < 0.0 || x < 0.0 || x >= a + 1) {
            throw new RuntimeException("Invalid a, x");
        }
        ...
        while (check) {
            ...
            if (i >= INCOMPLETE_GAMMA_MAX_ITERATIONS) {
                check = false;
                igf = sum * Math.exp(-x + acopy * Math.log(x) - loggamma);
                throw new RuntimeException("Gamma.regularizedIncompleteGammaSeries: Maximum number of iterations wes exceeded");
            }
        }
        return igf;
    }
    ...
    private static double regularizedIncompleteGammaFraction(double a, double x) {
        ...
        while (check) {
            ...
            if (i >= INCOMPLETE_GAMMA_MAX_ITERATIONS) {
                check = false;
                throw new RuntimeException("Gamma.regularizedIncompleteGammaFraction: Maximum number of iterations wes exceeded");
            }
        }
    ...
    public static double inverseRegularizedIncompleteGamma(double a, double p) {
        if (a <= 0.0) {
            throw new RuntimeException("a must be pos in invgammap");
        }
