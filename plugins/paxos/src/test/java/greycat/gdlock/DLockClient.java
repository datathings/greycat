...
public class DLockClient {
    ...
    public static void main(String[] args) {
        Graph g = new CoreGraph(new WSClient("ws://" + Config.master + ":8090/ws"), Config.cacheSize, Config.cacheSize, new HybridScheduler(), null, false);
                                ...
                                newTask()
                                        .thenDo(new ActionFunction() {
                                            ...
                                            public void eval(TaskContext ctx) {
                                                ...
                                                ((WSClient)g.storage()).extLock(n.id(), new Callback<Boolean>() {
                                                    @Override
                                                    public void on(Boolean result) {
                                                        r.externalLock(n);
                                                        n.set("name", Type.STRING, ctx.template("node_{{i}}"));
                                                        ((WSClient)g.storage()).extUnlock(n.id(), new Callback<Boolean>() {
                                                            @Override
                                                            public void on(Boolean result) {
                                                                r.externalUnlock(n);
                                                                n.free();
